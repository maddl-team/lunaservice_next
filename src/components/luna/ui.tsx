...
export function StatCard({ value, label, sublabel, withBorder }: { value: ReactNode; label: string; sublabel: string; withBorder?: boolean }) {
  return (
    <div className={`px-[32px] py-[32px] ${withBorder ? "border-r border-r-[rgba(0,0,0,0.08)]" : ""}`}>
      <div className="font-serif text-[56px] leading-[1] tracking-[-0.025em] text-[#161714]">{value}</div>
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-1 flex-col px-[24px] pb-[28px] pt-[24px]">
          ...
          <h3 className="m-0 font-serif text-[28px] leading-[1.05] tracking-[-0.02em]">{title}</h3>
  ...
  return (
    ...
    >
      <div className="px-[48px] py-[40px]">
        ...
        <h3 className="m-0 font-serif text-[64px] leading-[0.95] tracking-[-0.02em]">{title}</h3>
...
export function FeatureCard({ num, progress, title, body }: { num: string; progress: string; title: string; body: string }) {
  return (
    ...
    >
      ...
      <h3 className="m-0 font-serif text-[30px] leading-[1.1] tracking-[-0.015em]">{title}</h3>
...
export function ReviewCard({
  ...
  return (
    ...
    >
      ...
      <blockquote className="m-0 flex-1 font-serif text-[22px] leading-[1.4] tracking-[-0.005em]">&quot;{quote}&quot;</blockquote>
  ...
  return (
    <div className="overflow-hidden rounded-[20px] border border-[rgba(0,0,0,0.06)] bg-white" style={{ borderRadius: lunaRadii.sm, border: lunaEffects.borderSoft }}>
      <button onClick={onToggle} className="flex w-full items-center justify-between gap-[24px] bg-transparent px-[32px] py-[28px] text-left">
        <h3 className="m-0 font-serif text-[24px] leading-[1.3] tracking-[-0.01em]">{question}</h3>
        <span className={`inline-flex h-[36px] w-[36px] items-center justify-center rounded-full text-[18px] ${isOpen ? "bg-[#99cc33]" : "bg-[#fbf9f3]"}`}>
          {isOpen ? "–" : "+"}
        </span>
      </button>
...
export function InputField({ label, placeholder }: { label: string; placeholder: string }) {
  return (
    <div>
      ...
      <input
        className="w-full rounded-[14px] border border-[rgba(0,0,0,0.08)] bg-[#fbf9f3] px-[18px] py-[16px] text-[15px]"
...
export function TextareaField({ label, placeholder }: { label: string; placeholder: string }) {
  return (
    <div>
      <label className="mb-[8px] block text-[13px] font-medium text-[#161714]">{label}</label>
      <textarea
        className="min-h-[110px] w-full rounded-[14px] border border-[rgba(0,0,0,0.08)] bg-[#fbf9f3] px-[18px] py-[16px] text-[15px]"
        style={{ borderRadius: lunaRadii.xs, border: lunaEffects.borderMedium, fontSize: lunaTypography.body.sm }}
        placeholder={placeholder}
      />
    </div>
...
export function ChoiceChip({ label, active = false }: { label: string; active?: boolean }) {
  return (
    <label
      className={`flex cursor-pointer items-center gap-[12px] rounded-[14px] px-[16px] py-[14px] text-[14px] ${active ? "border border-[#1a1f0d] bg-[#1a1f0d] text-[#99cc33]" : "border border-[rgba(0,0,0,0.08)] bg-[#fbf9f3] text-[#161714]"}`}
  ...
  return (
    <div>
      ...
      <ul className="m-0 flex list-none flex-col gap-[10px] p-0 text-[14.5px]">
        {links.map((link) => (
          <li key={link} className="cursor-pointer text-[rgba(251,249,243,0.85)]">
            {link}
          </li>
        ))}
      </ul>
